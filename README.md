...
- `src/ecoscope-workflows-ext-general`: Contains custom tasks
...
## Task Development
The tasks are defined under `src/ecoscope-workflows-ext-general/ecoscope_workflows_ext_general/tasks`. Here's an example of a task
...
You can also add other dependencies in `src/ecoscope-workflows-ext-general/pyproject.toml` under [tool.pixi.dependencies] section

Unittests are under `src/ecoscope-workflows-ext-general/tests` and can be tested it by
```bash
pixi run pytest-310
```
...
2. Run
   ...
   ecoscope-workflows-ext-general = { version = "*", channel = "file:///tmp/ecoscope-workflows-custom/release/artifacts/" }
   ```
...
5. Update the version in pixi.toml
   ```toml
   [dependencies.ecoscope-workflows-ext-general]
